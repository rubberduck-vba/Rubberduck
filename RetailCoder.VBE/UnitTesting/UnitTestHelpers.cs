...
namespace Rubberduck.UnitTesting
{
    public static class UnitTestHelpers
    {
        ...
        {
            return GetTestModuleProcedures(state).Where(item => IsTestMethod(state, item)).Select(item => new TestMethod(item, vbe));
        }
        ...
        {
            if (state.AllUserDeclarations.Any(d => d.DeclarationType == DeclarationType.Parameter &&
                                                    d.ParentScopeDeclaration == item))
            {
                return false;
            }

            if (item.Annotations.Any(a => a.AnnotationType == AnnotationType.TestMethod))
            {
                return true;
            }

            var nontestMethodAnnotations = new[]
            {
                AnnotationType.ModuleCleanup,
                AnnotationType.ModuleInitialize,
                AnnotationType.TestCleanup,
                AnnotationType.TestInitialize
            };

            if (item.Annotations.Any(a => nontestMethodAnnotations.Contains(a.AnnotationType)))
            {
                return false;
            }

            if (item.IdentifierName.StartsWith("Test") &&
                item.IdentifierName != "TestInitialize" &&
                item.IdentifierName != "TestCleanup")
            {
                return true;
            }
        ...
        public static IEnumerable<QualifiedMemberName> FindModuleCleanupMethods(this QualifiedModuleName module, RubberduckParserState state)
        {
            return GetTestModuleProcedures(state).Where(m =>
                        ...
                        m.IdentifierName == "ModuleCleanup").Select(s => s.QualifiedName);
        }
        ...
        public static IEnumerable<QualifiedMemberName> FindTestInitializeMethods(this QualifiedModuleName module, RubberduckParserState state)
        {
            return GetTestModuleProcedures(state).Where(m =>
                        ...
                        m.IdentifierName == "TestInitialize").Select(s => s.QualifiedName);
        }
        ...
        public static IEnumerable<QualifiedMemberName> FindTestCleanupMethods(this QualifiedModuleName module, RubberduckParserState state)
        {
            return GetTestModuleProcedures(state).Where(m =>
                        ...
                        m.IdentifierName == "TestCleanup").Select(s => s.QualifiedName);
        }

        private static IEnumerable<Declaration> GetTestModuleProcedures(RubberduckParserState state)
        {
            var procedures = state.AllUserDeclarations
                        .Where(item => item.DeclarationType == DeclarationType.Procedure);

            return procedures.Where(item =>
                        item.ParentDeclaration.DeclarationType == DeclarationType.ProceduralModule &&
                        item.ParentDeclaration.Annotations.Any(
                            a => a.AnnotationType == AnnotationType.TestModule));
        }
    }
}